...
for fidx, fn in enumerate(fs):
    ...
    print "file name:%s, idx:%d" % (fn, fidx)
    img_bin = f.read()
...
for sample_idx, sample in enumerate(x2):
    print "let's go to sample:%d" % sample_idx
    ...
    for band in (np.arange(0.5, 0.1, -0.001).tolist() + np.arange(0.1, 0, -0.0002).tolist()):
        print "post-battle by band:%s" % band
        ...
        if min_error2 < min_error:
            print "new king showed up! min error:%s" % min_error2
            best_z = best_z2
            best_img = best_img2
            min_error = min_error2

    y.append(best_img)
    print xp.array(best_z)
